...
import com.songming.sanitation.workdeal.adapter.ApprovalResultListAdapter;
 ...
 *
 */
public class ApprovalResultListAcitivity extends BaseActivity implements
		OnClickListener, OnItemClickListener {

	private TextView topTitle;
	private LinearLayout layoutback;
	private ImageView layoutbackimg;
	private List<TJobAuditDto> data = new ArrayList<TJobAuditDto>();
	private ApprovalResultListAdapter adapter;
	private PullToRefreshListView listview;
	private int mCurIndex = 1;
	private static final String TAG = "ApprovalResultListAcitivity";

	@Override
	protected void onCreate(Bundle arg0) {
		super.onCreate(arg0);
		setContentView(R.layout.approvalresult_list_activity);
		findViews();
		initViews();
	...
	protected void initViews() {
		...
		adapter = new ApprovalResultListAdapter(this, null);
	...
	protected void successCallback(JSONObject jsonObject, Object requestTag) {
		...
		if (TAG.equals(requestStr)) {
			...
			if (flag.equals("false")) {
				...
				Gson gson = new Gson();
				List<TJobAuditDto> dataNew = gson.fromJson(data1,
						new TypeToken<List<TJobAuditDto>>() {
						}.getType());
				// 判断是否有数据可供加载
				if (mCurIndex <= 1) {
					data = dataNew;
	...
	public void onItemClick(AdapterView<?> parent, View view, int position,
			long id) {
		 Intent intent = new Intent(this, ApprovalResultAcitivity.class);
	...
	/**
	 * 请求数据
	 * param={"page":1,"rows":20,"paramsMap":{"createId":1}}
	 */
