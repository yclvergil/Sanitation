 ...
 * 
 */
public class ClockUnderlingActivity extends BaseActivity implements
		OnClickListener {
	...
	protected void initViews() {
		...
		listview.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				Intent intent = new Intent(ClockUnderlingActivity.this,
	...
	private void requestOrgList() {
		...
		long thistaffid = getIntent().getLongExtra("thistaffid", -1);
		JSONObject jsonObject = new JSONObject();
		try {
			jsonObject.put("staffId", thistaffid);
	...
	protected void successCallback(JSONObject jsonObject, Object requestTag) {
		...
		if ("contacts".equals(requestStr)) {
			...
			if (list.size() == 0 || list == null) {
				Toast.makeText(this, "没有下属了哦！", Toast.LENGTH_SHORT).show();

				new Timer().schedule(new TimerTask() {
					@Override
					public void run() {

						ClockUnderlingActivity.this.finish();
					}
				}, 1000, 1000);
